"""Registry binding for the seeded random long-short sanity baseline."""
...
from llca.models.estimators.baseline import RandomLongShortEstimator
from llca.pipeline.contracts import ModelCapabilities, ObjectiveKind, TrainingEngine


@model_registry.register_validator("random-long-short")
def validate(cfg: DictConfig) -> list[str]:
    """Validate the panel roles the random long-short baseline reads."""
    return validate_tabular_io(cfg)


@model_registry.register("random-long-short")
def build(
    cfg: DictConfig,
    *,
    ...
    **_: object,
    ...
    return partial(RandomLongShortEstimator, config=cfg, prediction_kind=kind)
