...
def fmg_data_requirements(
    model: DictConfig,
    *,
    input_scope: EntityScope,
    supervision_scope: EntityScope,
) -> DataRequirements:
    """Resolve FMG's named panel roles without coupling ingestion to the architecture."""
    inputs = model.inputs
    feature_name = str(inputs.features)
    context_names = [str(name) for name in as_list(inputs.context)]
    supervision_name = str(model.supervision.dataset)
    scopes: dict[str, EntityScope] = {
        feature_name: input_scope,
        **{name: input_scope for name in context_names},
    }
...
def validate_transformer_fmg(cfg: DictConfig) -> list[str]:
...
def validate_single_asset_allocation(
...
def validate_single_asset_objective(model_name: str, loss: nn.Module | None) -> None:
