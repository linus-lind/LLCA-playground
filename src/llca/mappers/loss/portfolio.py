...
_PORTFOLIO_FIELDS = [
    ...
    ConfigField("common_score_aversion", "number", minimum=0.0, required=False),
    ConfigField("net_exposure_aversion", "number", minimum=0.0, required=False),
    ConfigField("net_exposure_tolerance", "number", minimum=0.0, required=False),
...
def _build_portfolio(cfg: DictConfig, **_: object) -> nn.Module:
    ...
    return PortfolioLoss(
        ...
        common_score_aversion=float(cfg.get("common_score_aversion", 0.0)),
        net_exposure_aversion=float(cfg.get("net_exposure_aversion", 0.0)),
        net_exposure_tolerance=float(cfg.get("net_exposure_tolerance", 0.0)),
    )
...
def _validate_portfolio(cfg: DictConfig) -> list[str]:
    ...
    tolerance = cfg.get("net_exposure_tolerance", 0.0)
