...
from llca.analytics.evaluation.portfolio_attribution import (
    attribute_assets,
    attribute_maximum_drawdown,
    attribute_signal_buckets,
)
...
def _normalised_weights(
    ...
    """Turn dense scores into portfolio weights via the objective's normalization callback.

    ``scores`` and ``valid`` are both date-by-entity. Invalid cells are zeroed before the
    callback and forced back to exactly zero afterwards, so no weight is placed on an
    unobservable asset. Raises ``ValueError`` if the callback returns any non-finite weight.
    """
...
def _simple_returns(returns: pd.DataFrame, return_type: ReturnType) -> pd.DataFrame:
    """Express asset returns as simple returns, converting from log returns when needed.

    Portfolio accounting compounds arithmetically, so log inputs are mapped through
    ``expm1``. Raises ``ValueError`` if any resulting simple return is at or below -100%.
    """
...
def _turnover_and_costs(
    ...
    *,
    ...
    """Compute per-date turnover and trading costs from realized weight changes.

    Yesterday's weights are drifted forward by the assets' and residual cash's one-period
    growth (renormalized by portfolio NAV) before being compared with today's target, so the
    trade counted is the genuine rebalancing move, not passive drift. Returns a turnover frame
    (L1, one-way, and long/short leg turnover) and a cost frame where transaction costs scale
    with L1 turnover and borrow cost with short exposure. The opening trade is charged only
...
def _drawdown_path(returns: pd.Series) -> pd.DataFrame:
    """Trace the wealth path and its drawdown geometry from a period-return series.

    Returns a frame with compounded wealth, its running high-water mark (floored at the
    starting value of one), the fractional drawdown from that mark, and the number of
    consecutive periods spent underwater at each point.
    """
    wealth = (1.0 + returns).cumprod()
    high_water_mark = wealth.cummax().clip(lower=1.0)
...
def _run_lengths(mask: np.ndarray) -> list[int]:
    """Return the length of every maximal run of ``True`` values in a boolean array.

    A run still open at the end of the array contributes its length like any other.
    """
    if not mask.any():
        return []
    padded = np.concatenate(([False], mask, [False]))
    boundaries = np.flatnonzero(padded[1:] != padded[:-1])
    return [int(length) for length in boundaries[1::2] - boundaries[0::2]]


def _drawdown_episode_lengths(drawdown: pd.Series) -> list[int]:
    """List the length of every underwater stretch in a drawdown series.

    Each run of consecutive negative drawdowns contributes its length, including a final run
    still open at the end of the series.
    """
    return _run_lengths(drawdown.to_numpy() < 0.0)


def _maximum_consecutive(condition: pd.Series) -> int:
    """Return the length of the longest unbroken run of true values in ``condition``."""
    return max(_run_lengths(condition.to_numpy(dtype=bool)), default=0)


def _autocorrelation(returns: pd.Series, lag: int) -> float:
    """Return the lag-``lag`` autocorrelation of ``returns``, or ``nan`` when undefined.

    Pairs each observation with the one ``lag`` positions earlier and correlates the two
    slices. Yields ``nan`` if the series is too short or either slice has no dispersion.
    """
    if len(returns) <= lag:
        return float("nan")
...
def _rolling_tail_risk(
    ...
    """Compute rolling historical VaR and expected shortfall for each requested level.

    Works on the loss series (negated returns). For every ``level`` it emits a ``var_<pct>``
    column holding the rolling loss quantile and an ``expected_shortfall_<pct>`` column holding
    the mean loss beyond that quantile, over a trailing ``window`` of periods.
    """
    losses = -returns
...
def _performance_metrics(
    ...
    *,
    ...
    """Compute a broad performance panel for one return series, keyed under ``prefix``.

    Covers cumulative and annualized returns, volatility and downside deviation, the Sharpe,
    Sortino, Calmar, Omega and related ratios, drawdown depth and duration, win/loss shape,
    autocorrelations, worst rolling windows, and empirical VaR/expected shortfall. Returns are
    simple periodic; ``risk_free`` is a per-date rate so the Sharpe ratio uses realized excess
    return, while ``minimum_acceptable_return`` is an annual constant. Also returns the drawdown
    path. Raises if the series is empty or contains a return at or below -100%.
    """
    if returns.empty:
    ...
    excess = returns - align_risk_free(risk_free, returns.index)
    ...
    return_skewness, return_kurtosis = shape_statistics(returns)
    ...
    metrics = {
        ...
        f"{prefix}_sharpe_ratio": (
            excess_mean / excess_std * np.sqrt(annualization_periods)
            if excess_std > EPS
            else float("nan")
        ),
        ...
        f"{prefix}_calmar_ratio": (
            cagr / abs(maximum_drawdown) if maximum_drawdown < -EPS else float("nan")
        ),
        ...
        f"{prefix}_payoff_ratio": (
            ...
            if not wins.empty and not losing.empty and abs(losing.mean()) > EPS
            else float("nan")
        ),
    ...
    for window in worst_rolling_windows:
...
def _safe_sum_ratio(numerator: pd.Series, denominator: pd.Series) -> float:
    ...
    return float(numerator.sum()) / denominator_sum if denominator_sum > EPS else float("nan")


def _tail_ratio(returns: pd.Series) -> float:
    lower = abs(float(returns.quantile(0.05)))
    return float(returns.quantile(0.95)) / lower if lower > EPS else float("nan")
...
def _position_holding_period(weights: pd.DataFrame, active_threshold: float) -> float:
    """Return the mean number of consecutive periods a position stays active, across all assets.

    A position is active on dates where its absolute weight exceeds ``active_threshold``; each
    unbroken active stretch in every asset contributes its length to the average. Yields
    ``nan`` when no position is ever active.
    """
    durations: list[int] = []
    for column in weights:
        durations.extend(_run_lengths(weights[column].abs().to_numpy() > active_threshold))
...
def build_portfolio_evaluation(
    ...
    *,
    ...
    slippage: float,
    borrow_cost: float,
    autocorrelation_lags: tuple[int, ...] = (1, 5, 21),
    worst_rolling_windows: tuple[int, ...] = (5, 21, 63),
    target_threshold: float = 0.0,
) -> PortfolioEvaluation:
    """Run the full portfolio simulation and analytics for one model's scores.

    Scores and target returns are pivoted to a date-by-entity grid and turned into weights once
    by the objective's normalization callback; every downstream quantity — gross and net
    returns, exposures, turnover, costs, drawdowns, rolling statistics, monthly/yearly returns,
    and the asset, side, signal-bucket and drawdown attributions — is derived from that single
    weight path. Targets are converted to simple returns for accounting, and residual cash
    (one minus net exposure) earns the aligned risk-free rate. Before returning, the function
    asserts that the per-date leg contributions sum to gross return and that gross minus costs
    equals net return, raising ``RuntimeError`` if either identity is violated.
    """
    score_frame = _dense(scores, "value")
    raw_return_frame = _dense(target_returns, "value").reindex_like(score_frame)
    ...
    gross_exposure = absolute_weights.sum(axis=1)
    exposure_shares = absolute_weights.div(gross_exposure.replace(0.0, np.nan), axis=0)
    concentration = exposure_shares.pow(2).sum(axis=1, min_count=1)
    # Descending cumulative weight shares: column k-1 holds each date's top-k concentration, so a
    # single sort replaces a per-date ``nlargest`` apply for every reported threshold. Empty dates
    # (zero gross exposure) are all-NaN rows; treating them as zero matches ``nlargest`` summing an
    # empty selection to zero, and non-negative shares keep the sort order meaningful.
    sorted_shares = -np.sort(-np.nan_to_num(exposure_shares.to_numpy(dtype=float), nan=0.0), axis=1)
    cumulative_shares = np.cumsum(sorted_shares, axis=1)
    column_count = cumulative_shares.shape[1]
    composition = pd.DataFrame(
        {
            ...
            "effective_positions": 1.0 / concentration.clip(lower=EPS),
            "largest_absolute_weight": absolute_weights.max(axis=1),
            "top_5_weight_share": pd.Series(
                cumulative_shares[:, min(5, column_count) - 1], index=exposure_shares.index
            ),
            "top_10_weight_share": pd.Series(
                cumulative_shares[:, min(10, column_count) - 1], index=exposure_shares.index
            ),
        }
    )
    long_leg_contribution = (long_weights * asset_returns).sum(axis=1)
    short_leg_contribution = (short_weights * asset_returns).sum(axis=1)
    daily = pd.DataFrame(
        {
            ...
            "excess_net_return": net_returns - risk_free_daily,
            "long_return_contribution": long_leg_contribution,
            "short_return_contribution": short_leg_contribution,
            "long_leg_return": long_leg_contribution / exposures["long"].replace(0.0, np.nan),
            "short_leg_return": short_leg_contribution / exposures["short"].replace(0.0, np.nan),
        }
    )
    ...
    gross_metrics, _ = _performance_metrics(
        ...
        worst_rolling_windows=worst_rolling_windows,
    )
    ...
    rolling_excess = net_returns - risk_free_daily
    rolling_excess_mean = rolling_excess.rolling(rolling_window, min_periods=rolling_window).mean()
    rolling_excess_std = rolling_excess.rolling(rolling_window, min_periods=rolling_window).std(
        ddof=1
    )
    rolling_std = net_returns.rolling(rolling_window, min_periods=rolling_window).std(ddof=1)
    rolling = pd.DataFrame(
        {
            ...
            "sharpe_ratio": rolling_excess_mean
            / rolling_excess_std.replace(0.0, np.nan)
    ...
    rolling["short_exposure"] = (
        exposures["short"].rolling(rolling_window, min_periods=rolling_window).mean()
    )
    ...
    metrics |= {
        ...
        "annualized_cash_return_contribution": float(
            daily["cash_return_contribution"].mean() * annualization_periods
        ),
        ...
        "total_cash_return_contribution": float(daily["cash_return_contribution"].sum()),
    }
    ...
    side_attribution = pd.DataFrame(
        {
            "total_contribution": [
                ...
                daily["cash_return_contribution"].sum(),
            ...
            "mean_daily_contribution": [
                ...
                daily["cash_return_contribution"].mean(),
                -costs["transaction"].mean(),
                -costs["borrow"].mean(),
            ],
        },
        index=pd.Index(
            ["long", "short", "cash", "transaction_costs", "borrow_costs"], name="source"
        ),
    )
    asset_attribution = attribute_assets(
        contributions,
        weights,
        cash_contribution,
        cash_weight,
    ...
    if return_contribution_total > EPS:
    ...
    if risk_contribution_total > EPS:
    ...
    signal_attribution = attribute_signal_buckets(
        score_frame.where(valid),
        asset_returns.where(valid),
        weights,
        signal_buckets,
        target_threshold,
        active_weight_threshold,
    )
    drawdown_attribution = attribute_maximum_drawdown(
        drawdowns,
        contributions,
        cash_contribution,
        costs,
    ...
    gross_reconciliation = float(
        (
            daily["long_return_contribution"]
            + daily["short_return_contribution"]
            + daily["cash_return_contribution"]
            - gross_returns
        )
        .abs()
        .max()
