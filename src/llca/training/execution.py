"""Strategy-neutral experiment execution with resumable, idempotent MLflow lifecycle."""
...
from llca.training.manifests import RunManifests, validate_training_manifest
...
def _find_logged_model_uri(client: MlflowClient, experiment_id: str, run_id: str) -> str | None:
...
def _execute_fold[DataT](
    run: Run,
    fold: Fold,
    train: DataT,
    val: DataT,
    estimator_factory: Callable[[], Estimator[DataT]],
    ...
    *,
    ...
    manifests.log(
        include_invocation=not resume,
        include_source=not resume,
        include_environment=not resume,
    )
    ...
    if model_uri is None:
        ...
        estimator.fit(
            ...
            tracker=MlflowTrainingTracker(
                run_id,
                batch_log_interval=training.tracking_interval,
            ),
...
def _children_by_fold(client: MlflowClient, parent: Run) -> dict[int, Run]:
    ...
    children = [
        ...
        and run.data.tags.get(RUN_KIND_TAG) == RUN_KIND_FOLD
    ]
...
def _run_completed(run: Run) -> bool:
    return bool(run.data.tags.get(RUN_PHASE_TAG) == RUN_PHASE_COMPLETED)


def _execute_plan[DataT](
    parent: Run,
    panels: DataT,
    splitter: Splitter[DataT],
    estimator_factory: Callable[[], Estimator[DataT]],
    training: TrainingPolicy,
    primary_dataset: str,
    run_tags: Mapping[str, str] | None,
    ...
    if fold_count == 0:
        raise ValueError(
            "split configuration produced no complete train/validation/test fold; "
            "reduce split sizes or provide a longer primary dataset"
        )
...
def execute_training[DataT](
    ...
    *,
    ...
    """Execute any registered chronological split plan through one estimator lifecycle."""
    ...
    resolved_config = validate_training_manifest(resolved_config)
    resolved_data_manifest = validate_data_manifest(dict(data_manifest))
    manifests = RunManifests(
        training=resolved_config,
        data=resolved_data_manifest,
        ...
        environment=(dict(environment_manifest) if environment_manifest is not None else None),
    )
